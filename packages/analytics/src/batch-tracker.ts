...
export class EventBatchTracker {
	...
	flushBatchSync(): boolean {
		...
		if (!sessionId) {
			// Fall back to any session ID for sync flush (page unload scenario)
			sessionId = getSessionId(config.campaignId);
			if (!sessionId) {
				log(
					"❌ No session ID found for sync batch, cannot send (session expired and no renewal possible during page unload)",
				);
				return false;
			}
			log("⚠️ Using expired session ID for sync batch (page unload)");
		}
	...
	private async sendBatch(batchItems: BatchItem[]): Promise<boolean> {
		...
		if (!sessionId) {
			// Fall back to any session ID and let server handle renewal
			sessionId = getSessionId(config.campaignId);
			if (!sessionId) {
				...
				try {
					...
					const result = await renewSession(
						oldSessionId || "expired",
						newSessionId,
					);
					if (result.success) {
						sessionId = newSessionId;
						log("✅ Session renewed for batch:", sessionId);
					} else {
				...
				log("⚠️ Using expired session ID for batch, server will handle renewal");
			}
		}
		...
		if (result.new_session_id) {
			...
			if (renewalResult.success) {
				...
				// Retry the batch with renewed session - this will get the new session ID from cookies
