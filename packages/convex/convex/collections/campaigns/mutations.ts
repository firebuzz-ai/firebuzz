...
export const create = mutationWithTrigger({
	...
	handler: async (ctx, args) => {
		...
		const primaryGoal = {
			id: primaryGoalEvent.id,
			title: primaryGoalEvent.title,
			description: primaryGoalEvent.description,
			direction: primaryGoalEvent.direction,
			placement: primaryGoalEvent.placement,
			value: primaryGoalEvent.value,
			currency: primaryGoalEvent.currency,
			type: primaryGoalEvent.type,
			icon: primaryGoalEvent.icon,
			isCustom: false,
		};
		...
		if (type === "preview") {
			...
			if (campaign.status === "published") {
				...
				await ctx.db.patch(id, {
					status: "preview",
					previewPublishedAt: new Date().toISOString(),
					previewFirstPublishedAt:
						campaign.previewFirstPublishedAt || new Date().toISOString(),
					urlConfig: {
						previewUrl,
					},
					updatedAt: new Date().toISOString(),
			...
			await ctx.db.patch(id, {
				...
				productionFirstPublishedAt:
					campaign.productionFirstPublishedAt || new Date().toISOString(),
				urlConfig: {
					previewUrl:
...
export const updateCampaignSettings = mutationWithTrigger({
	args: {
		...
		campaignSettings: v.object({
			primaryGoal: v.optional(eventSchemaConvex),
			customEvents: v.optional(v.array(eventSchemaConvex)),
			gdpr: v.optional(gdprSchemaConvex),
	...
	handler: async (ctx, args) => {
		...
		// Validate CTA link URL format
		if (
			args.campaignSettings.ctaLink !== undefined &&
			args.campaignSettings.ctaLink !== ""
		) {
			try {
				new URL(args.campaignSettings.ctaLink);
			} catch {
				throw new ConvexError("CTA link must be a valid URL");
			}
		}

		// Merge with existing campaign settings
		const updatedSettings: Doc<"campaigns">["campaignSettings"] = {
