...
export const revalidateAnalytics = mutation({
	...
	handler: async (ctx, args) => {
		...
		for (const queryParams of args.queries) {
			try {
				// Get the existing analytics pipe using appropriate index
				let existingAnalyticsPipe: Doc<"analyticsPipes"> | null = null;

				if (queryParams.queryId === "ab-test-result") {
					// Use AB test specific index for AB test results
					existingAnalyticsPipe = await ctx.db
						.query("analyticsPipes")
						.withIndex("by_campaign_abtest_params", (q) =>
							q
								.eq("campaignId", args.campaignId)
								.eq("queryId", queryParams.queryId)
								.eq("abTestId", queryParams.abTestId)
								.eq("conversionEventId", queryParams.conversionEventId)
								.eq("campaignEnvironment", queryParams.campaignEnvironment),
						)
						.first();
				} else {
					// Use period-based index for other query types
					existingAnalyticsPipe = await ctx.db
						.query("analyticsPipes")
						.withIndex("by_campaign_query_params", (q) =>
							q
