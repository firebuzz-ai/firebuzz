...
app.all('*', async (c) => {
	...
	const previewHostname = firebuzzPreviewHostnames.find((previewHostname) => previewHostname.hostname === hostname);
	...
	const targetWorkerUrl = workers[config.e as keyof typeof workers] || workers.production;
