...
export const Canvas = ({
	...
	// Handle selection changes to sync with URL
	const onSelectionChange = useCallback(
		({ nodes: selectedNodes }: { nodes: Node[]; edges: Edge[] }) => {
			...
			if (selectedNodes.length > 0) {
				...
				const nodeScreen = lastSelectedNode.type as CampaignScreen;
				setNavigationState({ screen: nodeScreen });
			} else {
				console.log("THIS2");
				...
				if (sourceNode?.type === "ab-test") {
					...
					if (remainingVariants.length > 0) {
						...
						const edgeChanges = edges
							.filter(
								(ed) =>
									ed.source === sourceNodeId && ed.target !== targetNodeId,
							)
							.map((ed, index) => ({
