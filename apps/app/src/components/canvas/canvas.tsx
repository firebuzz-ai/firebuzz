...
export const Canvas = ({
  ...
  onSaveHandler: (nodes: Node[], edges: Edge[]) => void;
}) => {
  ...
  const debouncedCallback = useDebouncedCallback(onSaveHandler, 3000);
  ...
  // Handle onChange
  const onNodesChangeHandler: OnNodesChange<Node> = async (changes) => {
    onNodesChange(changes);
    debouncedCallback(nodes, edges);
  };
