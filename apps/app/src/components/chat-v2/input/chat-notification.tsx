...
import { Avatar, AvatarFallback, AvatarImage } from "@firebuzz/ui/components/ui/avatar";
...
export const ChatNotification = () => {
				...
				action: {
					...
					onClick: async () => {
						try {
							...
							const errorMessage =
								error instanceof Error ? error.message : "Failed to restart sandbox";
							toast.error(errorMessage);
						}
					},
				},
	...
	}, [isActive, creditBalance, session, sandboxStatus, renewSession, renewSandbox]);
	...
	return (
		<AnimatePresence mode="wait">
			...
			>
				...
				{notification.type !== "message-queue" && <div className="flex justify-center items-center p-1 rounded-md border bg-muted">
					{getIcon(notification.type)}
				</div>}

				<div className="flex-1">
					{/* Regular Notification */}
				{notification.type !== "message-queue" && <div className="text-sm font-medium">{notification.title}</div>}
				{notification.type === "message-queue" && <div className="flex gap-2 items-center pt-1 text-xs font-medium">{<div className="flex justify-center items-center p-1 rounded-md border bg-muted">
					{getIcon(notification.type)}
													...
													<TooltipTrigger asChild>
														<div className="flex gap-2 items-center">
															<Avatar className="size-5">
																<AvatarImage src={`${NEXT_PUBLIC_R2_PUBLIC_URL}/${creator?.imageKey}`} />
				...
				{notification.link && (
					<Button variant="ghost" size="sm" className="text-xs shrink-0" asChild>
						<Link href={notification.link.href}>{notification.link.label}</Link>
					</Button>
				)}
			</div>
			</motion.div>
