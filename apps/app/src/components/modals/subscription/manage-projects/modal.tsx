...
export const ManageProjectsModal = () => {
	...
	// Get base project limit (without add-ons)
	const baseProjectLimit = useMemo(() => {
		return isTeamPlan ? 3 : 1;
	}, [isTeamPlan]);

	// Current extra projects from add-ons (reactive via Convex)
	const currentExtraProjects = useMemo(() => {
		return Math.max(0, projectLimit - baseProjectLimit);
	}, [projectLimit, baseProjectLimit]);
	...
	const newProjectLimit = useMemo(() => {
		return baseProjectLimit + newExtraProjects;
	}, [baseProjectLimit, newExtraProjects]);
	...
	const pricePerProject = useMemo(() => {
		if (!monthlyPrice?.unitAmount) return 0;
		return monthlyPrice.unitAmount / 100;
	}, [monthlyPrice]);
	...
	const interval = useMemo(() => {
		return monthlyPrice?.interval || "month";
	}, [monthlyPrice]);
						...
						<div className="px-4 py-4 space-y-4">
							...
							<div className="p-4 rounded-lg border bg-muted">
								<div className="space-y-2">
									<div className="flex justify-between items-center">
										...
										<span className="text-sm">{baseProjectLimit}</span>
									</div>
									<div className="flex justify-between items-center">
							...
							<div className="space-y-3">
								...
								<div className="text-xs text-muted-foreground">
									Total projects: {newProjectLimit} ({baseProjectLimit} base +{" "}
