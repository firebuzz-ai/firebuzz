...
import { SquareDashedMousePointer, X } from "@firebuzz/ui/icons/lucide";
import {
	IconArrowUp,
	IconBook,
	IconComponents,
...
export const ChatInput = ({ landingPageId }: ChatInputProps) => {
	...
	const { sandboxStatus, isPreviewIframeLoaded } = useSandbox();
	...
	const isDisabled =
		...
		!session ||
		session.status === "completed"; // TODO: Add credit balance check, subscription check
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		// Check if element is selected in design mode
		let finalPrompt = inputValue;
		if (isDesignModeActive && selectedElement) {
			// Wrap user message in element reference div (only essential data)
			const sanitizedInput = sanitizeUserInput(inputValue);
			finalPrompt = `<div data-element-reference="true" data-tag-name="${selectedElement.tagName}" data-source-file="${selectedElement.sourceFile}" data-source-line="${selectedElement.sourceLine}" data-source-column="${selectedElement.sourceColumn}">

${sanitizedInput}

</div>`;

			// Disable design mode after sending
			await disableDesignMode();
		} else {
			finalPrompt = sanitizeUserInput(inputValue);
		}
					...
					<Field>
						...
						>
							...
							>
								...
								{/* Element Reference Badge - only show when element is selected in design mode */}
								{isDesignModeActive && selectedElement && (
									<div className="flex gap-1 items-center px-2 py-1 h-8 text-sm text-blue-500 rounded-2xl border border-blue-500 bg-blue-500/20">
							...
							<InputGroupAddon align="block-end" className="gap-1">
								...
								<Tooltip>
									<TooltipTrigger asChild>
										...
										>
											<SquareDashedMousePointer className="size-3.5" /> Design
										</InputGroupButton>
									</TooltipTrigger>
									<TooltipContent>
										{session?.status === "completed"
