...
interface ThemeColorsControlsProps {
	themes: ThemeData[];
	onColorClick: (color: {
		name: string;
		displayName: string;
...
export const ThemeColorsControls = ({
	themes,
	onColorClick,
}: ThemeColorsControlsProps) => {
	...
	return (
		<div>
			{organizedThemes.map((themeData) => {
				...
				return (
					<div key={theme} className="px-2 py-4 space-y-4 border-b">
						{/* Theme Header */}
						<div className="flex justify-between items-center">
							<div>
								<h2 className="flex gap-2 items-center text-sm font-medium capitalize">
									{theme} Theme{" "}
								</h2>
								<p className="text-xs text-muted-foreground">
									Click colors to customize and see real-time preview.
								</p>
							</div>
						</div>
						...
						<div className="">
							{primaryColor ? (
								<button
									...
									className={cn(
										"flex relative gap-3 items-center p-3 w-full rounded-lg border transition-all duration-200 group bg-muted",
								...
								>
									<div
										className="w-10 h-10 rounded-lg border shadow-sm border-border/40 shrink-0"
										style={{
								...
								<div className="p-3 text-sm rounded-lg border border-dashed text-muted-foreground bg-muted/50">
																...
																>
																	<TooltipTrigger asChild>
																		<button
																			...
																			className={cn(
																				"flex relative gap-2 items-center p-2 rounded-md border transition-all duration-200 group border-border/40",
																				"hover:border-border hover:shadow-sm hover:bg-muted/30",
																				"focus:outline-none focus:ring-2 focus:ring-secondary focus:ring-offset-2 focus:ring-offset-background",
																				"active:scale-98 active:duration-75",
																			)}
																		>
																			<div
																				className="w-5 h-5 rounded-sm border shadow-sm border-border/40 shrink-0"
																				style={{
																	...
																	<TooltipContent
																		side="top"
																		className="text-xs font-medium border backdrop-blur-sm bg-popover/95 border-border/50"
																	>
