...
export const AudienceLanguagesChart = ({ audienceData }: AudienceLanguagesChartProps) => {
	const chartData = useMemo((): HorizontalBarChartData[] => {
		// Use real languages data from audience breakdown first
		if (audienceData?.payload.languages && audienceData.payload.languages.length > 0) {
			return audienceData.payload.languages
				.map(([language, count], index) => {
					const languageName = LANGUAGE_NAMES[String(language)] ||
						String(language).charAt(0).toUpperCase() + String(language).slice(1);
					return {
						name: languageName,
		...
		if (audienceData?.payload.countries && audienceData.payload.countries.length > 0) {
		...
		if (audienceData?.payload.browsers && audienceData.payload.browsers.length > 0) {
