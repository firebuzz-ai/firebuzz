...
export const FieldSettingsView = ({
	...
	const isCampaignPublished = campaign?.isPublished;

	// Canvas-only approach - update nodes through React Flow
	...
	// Local state for option editing (replaces Jotai)
	...
	// Title and description will be edited via regular inputs below, not inline in the header

	// Get the selected field based on selectedFieldId
	const selectedField = useMemo(() => {
		return formFields.find((field) => field.id === selectedFieldId) || null;
	...
	const saveFieldChanges = () => {
		...
		if (!selectedField || !formNode || !formId) return;
			...
			nodeData.schema?.map((field) => {
				if (field.id === selectedField.id) {
					return {
						...
						unique: data.unique ?? field.unique,
	...
	// Title and description are saved via saveFieldChanges triggered by input blur
	...
	const handleDeleteField = () => {
		...
		const nodeData = formNode.data as unknown as FormNodeData;
		const updatedSchema =
			nodeData.schema?.filter((field) => field.id !== selectedField.id) || [];
		...
		<div className="flex flex-col h-full">
			...
			<div className="flex flex-shrink-0 gap-3 items-center p-4 border-b bg-muted">
				...
				<div className="flex-1">
					<div className="flex flex-col">
						<div className="text-lg font-semibold leading-tight">
							{selectedField.title || "Untitled Field"}
						</div>
						<div className="text-sm leading-tight text-muted-foreground">
