...
export const AudienceContinentsChart = ({
	...
	const chartData = useMemo((): HorizontalBarChartData[] => {
		...
		if (
			audienceData?.payload.continents &&
			audienceData.payload.continents.length > 0
		) {
			return audienceData.payload.continents
				.map(([continent, count], index) => ({
					name:
						String(continent).charAt(0).toUpperCase() +
						String(continent).slice(1),
		...
		const countries =
			audienceData?.payload.countries || realtimeData?.payload.countries || [];
