...
interface AudienceTimezonesChartProps {
	...
	timeseriesData?: Extract<Doc<"analyticsPipes">, { queryId: "timeseries-primitives" }> | null;
}
...
const getTimezoneFromHour = (hour: number): string => {
	...
	const timezones = [
		"UTC-12", "UTC-11", "UTC-10", "UTC-9 (Alaska)", "UTC-8 (Pacific)", "UTC-7 (Mountain)",
		"UTC-6 (Central)", "UTC-5 (Eastern)", "UTC-4 (Atlantic)", "UTC-3 (Argentina)",
		"UTC-2", "UTC-1", "UTC+0 (GMT)", "UTC+1 (CET)", "UTC+2 (EET)", "UTC+3 (Moscow)",
		"UTC+4", "UTC+5", "UTC+6", "UTC+7", "UTC+8 (China)", "UTC+9 (Japan)", "UTC+10 (Australia)", "UTC+11"
	];
...
export const AudienceTimezonesChart = ({
	...
	const chartData = useMemo((): HorizontalBarChartData[] => {
		...
		if (audienceData?.payload.timezones && audienceData.payload.timezones.length > 0) {
		...
		if (audienceData?.payload.hourly_distribution && audienceData.payload.hourly_distribution.length > 0) {
		...
		if (timeseriesData?.payload && timeseriesData.payload.length > 0) {
			...
			timeseriesData.payload.forEach(dataPoint => {
				...
				hourlyActivity[hour] = (hourlyActivity[hour] || 0) + dataPoint.all_sessions;
			});
