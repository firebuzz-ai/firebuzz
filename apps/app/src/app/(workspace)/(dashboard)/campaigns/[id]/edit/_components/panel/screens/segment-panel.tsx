...
export const SegmentPanel = ({ node, campaign }: SegmentPanelProps) => {
  ...
  const [_translationState, { openModal: openTranslationModal }] =
    useNewTranslationModal();
  ...
  // Fetch translations for the current primary landing page
  const availableTranslations = useCachedQuery(
  ...
  // Get translations from the segment node data
  ...
  const updateSegmentData = useCallback((updates: Partial<typeof node.data>) => {
    updateNodeData(node.id, updates);
  }, [updateNodeData, node.id]);

  const updatePrimaryLandingPage = async (landingPageId: string) => {
    // Prevent unselecting once a landing page is selected
    if (!landingPageId && node.data.primaryLandingPageId) {
      return;
    }

    const newPrimaryLandingPageId = landingPageId
      ? (landingPageId as Id<"landingPages">)
      : undefined;

    // Update the primary landing page
    updateSegmentData({
      primaryLandingPageId: newPrimaryLandingPageId,
    });

    // If we're changing to a new landing page and translations are enabled,
    // we'll let the useEffect handle syncing translations
  };

  // Sync translations when primary landing page changes or translations are fetched
  useEffect(() => {
    if (!node.data.primaryLandingPageId || translationMode === "disabled") {
      // Clear translations if no primary page or translations disabled
      if (node.data.translations && node.data.translations.length > 0) {
        updateSegmentData({ translations: [] });
      }
      return;
    }

    // If we have available translations, sync them with the node data
    if (availableTranslations) {
      const translationsData = availableTranslations.map((translation) => ({
        landingPageId: translation._id,
        language: translation.language || "unknown",
      }));

      // Only update if the translations have changed
      const currentTranslations = node.data.translations || [];
      const translationsChanged =
        translationsData.length !== currentTranslations.length ||
        translationsData.some(
          (newTrans, index) =>
            currentTranslations[index]?.landingPageId !==
              newTrans.landingPageId ||
            currentTranslations[index]?.language !== newTrans.language
        );

      if (translationsChanged) {
        updateSegmentData({ translations: translationsData });
      }
    }
  }, [
              ...
              {translationMode !== "disabled" &&
                node.data.primaryLandingPageId && (
                  <Button
                    ...
                    onClick={() =>
                      openTranslationModal(node.data.primaryLandingPageId!)
                    }
                  >
              ...
              (translations && translations.length > 0 ? (
                <div className="grid grid-cols-1 gap-2">
                  {translations.map((translation) => (
                    <div
                      key={translation.landingPageId}
                      onClick={() => {
                        window.location.href = `/assets/landing-pages/${translation.landingPageId}/edit`;
                      }}
                      onMouseEnter={() =>
                        handleABTestHover(translation.landingPageId)
                      }
