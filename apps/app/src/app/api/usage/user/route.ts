...
export async function GET(request: NextRequest) {
	try {
		...
		const response = await fetch(
			`${TINYBIRD_BASE_URL}/v0/pipes/user_usage_for_workspace.json?workspaceId=${validatedParams.workspaceId}&periodStart=${validatedParams.periodStart.replace("Z", "")}&periodEnd=${validatedParams.periodEnd.replace("Z", "")}`,
			{
				headers: {
