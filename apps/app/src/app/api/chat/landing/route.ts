...
export async function POST(request: NextRequest) {
  ...
  const { projectId, currentFileTree, currentImportantFiles, currentBrand } =
    requestBody;
  ...
  try {
    // Build system prompt components with strategic caching
    const baseSystemPrompt = getSystemPrompt(`./${projectId}`);

    const brandInfo = currentBrand
      ? stripIndents(`
      // CURRENT BRAND INFORMATION
      Brand Name: ${currentBrand.name || "Not specified"}
      Website: ${currentBrand.website || "Not specified"}
      Description: ${currentBrand.description || "Not specified"}
      Persona: ${currentBrand.persona || "Not specified"}
      Email: ${currentBrand.email || "Not specified"}
      Phone: ${currentBrand.phone || "Not specified"}
      Address: ${currentBrand.address || "Not specified"}
    `)
      : "// CURRENT BRAND INFORMATION\nNo brand information available";

    const currentStatus = stripIndents(`
