...
export interface TrackingTokenPayload {
	sessionId: string;
	userId: string;
	attributionId?: string; // Include attribution ID in tracking token
	campaignId: string;
	workspaceId: string;
	projectId: string;
...
export async function verifyTrackingToken(
	...
	try {
		...
		// Decode payload
		const payload: TrackingTokenPayload = JSON.parse(
...
function base64UrlDecode(data: string): string {
	...
	return atob(paddedData.replace(/-/g, "+").replace(/_/g, "/"));
}
