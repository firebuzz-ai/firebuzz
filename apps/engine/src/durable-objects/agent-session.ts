...
export class AgentSessionDurableObject extends DurableObject<Env> {
	...
	> {
		try {
			...
			return { success: true, expiresAt: state.expiresAt, nextIdleCheck: nextIdleCheck };
		} catch (error) {
	 ...
	 */
	async scheduleSessionOver(params: { type: 'idle' | 'max-duration' }): Promise<{ success: boolean }> {
		...
		try {
			...
			console.error('[DO] Failed to notify Convex, will retry on next alarm:', errorMessage);
			return { success: false };
		}
	...
	async updateActivity(): Promise<{ success: boolean; error?: string }> {
		...
		console.log('[DO] Activity updated, lastActivity:', new Date(state.lastActivity).toISOString());
	...
	async alarm(): Promise<void> {
		...
		if (state.status === 'ended') {
			console.log('[DO] Session already ended, skipping alarm. Calling cleanup...');
			await this.cleanup();
			return;
		}
		...
		if (state.status === 'idle-warning') {
			...
			if (shouldEnd) {
				console.log('[DO] User did not continue after idle warning, ending session');
				await this.endSession('idle-timeout');
				return;
			}
			// User continued, reset to active and continue monitoring
			console.log('[DO] User continued after idle warning, resuming monitoring');
			state.status = 'active';
			await this.ctx.storage.put<AgentSessionState>('state', state);
		}
	 ...
	 */
	private async callConvex(endpoint: string, body: Record<string, unknown>): Promise<void> {
		try {
