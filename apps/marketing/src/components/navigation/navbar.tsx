...
export const Navbar = () => {
	...
	const updateIndicatorPosition = React.useCallback(() => {
		...
		const activeNavIndex = navLinks.findIndex((link) => {
			// Exact match for external links and home
			if (link.href.startsWith("http") || link.href === "/") {
				return link.href === pathname;
			}
			// For internal routes, check if pathname starts with the link href
	...
	const setNavLinkRef = (index: number) => (el: HTMLAnchorElement | null) => {
		...
		// Trigger indicator update when all refs are set
	...
	// Update when refs are set (after component mounts)
	React.useEffect(() => {
		if (navContainerRef.current && navLinksRef.current.length > 0) {
			updateIndicatorPosition();
		}
	}, [updateIndicatorPosition]);
	...
	React.useEffect(() => {
		...
		const handleLoad = () => updateIndicatorPosition();
